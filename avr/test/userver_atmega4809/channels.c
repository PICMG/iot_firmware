...
#define DEFINE_DIGITAL_IN_CHANNEL_FUNCTS(channelname, port, bit) \
    ...
    void CONCATENATE(channelname, _init()) { \
        ...
        (PORT ## port ## _DIR) &= (~(1<<bit)); \
        /* turn on the pull-up resistor */ \
        (PORT ## port ## _PIN ## bit ## CTRL) = PORT_PULLUPEN_bm; \
    } \
    void CONCATENATE(channelname, _sample()) { \
      CONCATENATE(channelname,_rawdata) = ((PORT ## port ## _IN)>>bit)&1; \
    } \
...
#define DEFINE_DIGITAL_OUT_CHANNEL_FUNCTS(channelname, port, bit) \
    ...
    void CONCATENATE(channelname, _setOutput(unsigned char output)) { \
        ...
        if (output) (PORT ## port ## _OUTSET) = (1<<bit); \
        else (PORT ## port ## _OUTCLR) = (1<<bit); \
    } \
  ...
  // declare the channel-related raw data
    ...
    DEFINE_DIGITAL_IN_CHANNEL_FUNCTS(digital_in1, A, 0)
  #endif
  #ifdef CHANNEL_DIGITAL_IN2
    DEFINE_DIGITAL_IN_CHANNEL_FUNCTS(digital_in2, A, 1)
  #endif
  #ifdef CHANNEL_IN_DIGITAL_IN3
    DEFINE_DIGITAL_IN_CHANNEL_FUNCTS(digital_in3, A, 2)
  #endif
  #ifdef CHANNEL_IN_DIGITAL_IN4
    DEFINE_DIGITAL_IN_CHANNEL_FUNCTS(digital_in4, A, 3)
  #endif
  #ifdef CHANNEL_IN_DIGITAL_IN5
    DEFINE_DIGITAL_IN_CHANNEL_FUNCTS(digital_in5, A, 4)
  #endif
  #ifdef CHANNEL_DIGITAL_OUT1
    DEFINE_DIGITAL_OUT_CHANNEL_FUNCTS(digital_out1, A, 5)
  #endif
  #ifdef CHANNEL_DIGITAL_OUT2
    DEFINE_DIGITAL_OUT_CHANNEL_FUNCTS(digital_out2, A, 2)
  #endif
  #ifdef CHANNEL_DIGITAL_OUT3
    DEFINE_DIGITAL_OUT_CHANNEL_FUNCTS(digital_out3, A, 3)
  #endif
  #ifdef CHANNEL_DIGITAL_OUT4
    DEFINE_DIGITAL_OUT_CHANNEL_FUNCTS(digital_out4, A, 4)
  #endif
  #ifdef CHANNEL_INTERLOCK_IN
    DEFINE_DIGITAL_IN_CHANNEL_FUNCTS(interlock_in, A, 7)
  #endif
  #ifdef CHANNEL_INTERLOCK_OUT
    DEFINE_DIGITAL_OUT_CHANNEL_FUNCTS(interlock_out, A, 7)
  #endif
    ...
    DEFINE_DIGITAL_IN_CHANNEL_FUNCTS(trigger_in, B, 0)
  #endif
  #ifdef CHANNEL_TRIGGER_OUT
    DEFINE_DIGITAL_OUT_CHANNEL_FUNCTS(trigger_out, B, 1)
  #endif
